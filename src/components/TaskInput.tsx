...
export const TaskInput = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (input.trim()) {
      onAddTask(input.trim(), selectedClient === "no_client" ? undefined : selectedClient, selectedType, executionDate, enableReminder ? reminderDate : undefined, enableReminder ? reminderFrequency : undefined, description.trim() || undefined);
      setInput("");
      setDescription("");
  ...
  return <form onSubmit={handleSubmit} className="space-y-4 w-full max-w-2xl mx-auto">
      <div className="flex gap-2">
        <Input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Agregar una nueva tarea..." className="flex-1 dark:bg-gray-800 dark:text-white" />
        <Button type="button" variant="outline" size="icon" onClick={startListening} className={`transition-colors dark:bg-gray-800 dark:text-white ${isListening ? "bg-accent text-accent-foreground" : ""}`}>
          <Mic className="h-4 w-4" />
        </Button>
        <Button type="submit" size="icon" className="bg-zinc-500 hover:bg-zinc-400">
          <Send className="h-4 w-4" />
        </Button>
      </div>

      <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Descripción (opcional)" className="dark:bg-gray-800 dark:text-white" />
      ...
      <div className="flex flex-wrap gap-2">
          ...
          <Select value={selectedClient} onValueChange={setSelectedClient}>
            ...
            <SelectContent>
              ...
              <div className="px-2 py-1">
                <Input placeholder="Buscar cliente..." value={clientSearch} onChange={e => setClientSearch(e.target.value)} className="mb-2" />
              </div>
              {filteredClients.map(client => <SelectItem key={client.id} value={client.id}>
                  {client.name}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0 dark:bg-gray-800">
            <CalendarComponent mode="single" selected={executionDate} onSelect={setExecutionDate} initialFocus />
          </PopoverContent>
        </Popover>
        ...
        {enableReminder && <>
            <Popover>
              ...
              <PopoverContent className="w-auto p-0 dark:bg-gray-800">
                <CalendarComponent mode="single" selected={reminderDate} onSelect={setReminderDate} initialFocus />
              </PopoverContent>
            </Popover>
