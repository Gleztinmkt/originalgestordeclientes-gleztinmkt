...
export const CalendarView = ({
  ...
  // Performance optimization: memoize selected month start/end for query optimization
  ...
  const {
    ...
    queryKey: ['publications', selectedClient, dateRange.start, dateRange.end],
    queryFn: async () => {
      ...
      // Optimization: Only fetch publications for the visible month
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      progress += 10; // Speed up loading progress
      if (progress > 100) {
        clearInterval(interval);
      } else {
        setLoadingProgress(progress);
      }
    }, 30); // Reduce interval time
    return () => clearInterval(interval);
  }, []);
  
  // Performance optimization: memoize filtered publications
  ...
  // Performance optimization: memoize calendar days
  const calendarDays = useMemo(() => {
    ...
    return { days, emptyDays, allDays: [...emptyDays, ...days] };
  }, [dateRange]);
  ...
  // Memoize components that don't need to be recreated on every render
  ...
  ), [clients, designers, selectedClient, selectedDesigner, selectedStatus, selectedType, selectedPackage, refetchDesigners, userRole]);
    ...
    <DragDropContext onDragEnd={handleDragEnd}>
      {isMobile ? (
        ...
        <div className="grid grid-cols-7 gap-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
          ...
          {calendarDays.allDays.map((date, index) => {
            if (!date) {
  ...
  ), [calendarDays, filteredPublications, clients, designers, isMobile, highlightedPublicationId, refetch, handleDragEnd]);
    ...
    <div className="h-screen flex flex-col">
      {isMobile ? (
        ...
        <>
          ...
          <div className="flex-1 p-4 overflow-auto touch-scroll">
            <CalendarContent />
          </div>
        </>
      )}
