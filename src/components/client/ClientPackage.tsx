  ...
  const generateCalendarImage = useCallback(async () => {
    ...
    try {
      ...
      try {
        const {
          data: publications = []
        } = await supabase.from('publications').select('*').eq('client_id', clientId).eq('package_id', packageId).is('deleted_at', null).order('date', {
          ascending: true
        });
  ...
  return <Card className="bg-white dark:bg-gray-800 border dark:border-gray-700">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Switch checked={paid} onCheckedChange={onUpdatePaid} disabled={disabled} />
          </div>
          <DropdownMenu>
      ...
      <CardContent>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex gap-2">
            {usedPublications === totalPublications && <Button onClick={handleSendCompletionMessage} className="w-full gap-2" variant="outline" disabled={disabled}>
                <Send className="h-4 w-4" />
                Enviar mensaje de completado
              </Button>}
            <PublicationCalendarDialog clientId={clientId} clientName={clientName} packageId={packageId} packageName={packageName} packageMonth={month} clientMaterialUrl={clientMaterialUrl} clientGeneralUrl={clientGeneralUrl} totalPublications={totalPublications} />
          </div>
        </div>
      </CardContent>
      ...
      if (!disabled) {
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <AddPackageForm onSubmit={handleEditSubmit} defaultValues={{
          packageType: "basico",
          month: month,
          paid: paid
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setIsDeleteDialogOpen(false)} className="text-slate-50">Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmDelete} className="bg-red-500 hover:bg-red-600" disabled={disabled}>
              Eliminar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
