...
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbyahM4qzOdRWIC1Sr3Xh1IArjk0BR1BKfzzFXKVESL1ovEEwV7NA-Wp7C75RP4ygCvovw/exec";

const parsePublicationLinks = (links: string | null): PublicationLink[] => {
  if (!links) return [];
  ...
  try {
    ...
    return [{ label: "enlace", url: links }];
  }
};

const upsertMaterialLink = async (publicationId: string, materialUrl: string) => {
  const { data: currentPub, error } = await supabase
    .from("publications")
    .select("links")
    .eq("id", publicationId)
    .single();
  ...
  const currentLinksArray = parsePublicationLinks(currentPub?.links ?? null);
  const alreadyHasMaterialLink = currentLinksArray.some(
    (link) => link.label === "material" && link.url === materialUrl,
  );

  if (alreadyHasMaterialLink) return false;

  currentLinksArray.push({ label: "material", url: materialUrl });
...
export const PublicationCalendarDialog = ({
  ...
  const handleCloseAttempt = useCallback(
    ...
    [editingPublication, hasUnsavedChanges, isSubmitting],
  );
  ...
  const handleTogglePublished = async (publicationId: string, isPublished: boolean) => {
    try {
      ...
      toast({
        title: isPublished
          ? "Publicación marcada como subida"
          : "Publicación marcada como pendiente",
  ...
  const generateWordCalendar = async () => {
    try {
                              ...
                              children: [
                                ...
                                new TableCell({
                                  children: [
                                    new Paragraph({
                                      children: [
                                        new TextRun({ text: pub.name, size: 16 }),
                                      ],
                                    }),
                                  ],
                                }),
      ...
      toast({
        title: "Calendario descargado",
        description: "El calendario en formato Word se ha descargado correctamente.",
      });
    } catch (error) {
  ...
  const handleGenerateDriveFolders = useCallback(
    async (selectedIds: string[]) => {
      setDriveStatus("loading");
      setDriveError(null);

      const selectedPubs = publications.filter((publication) =>
      ...
      try {
        const response = await fetch(GOOGLE_SCRIPT_URL, {
          ...
          body: JSON.stringify({
            ...
            publicaciones: selectedPubs.map((publication) => ({
              id: publication.id,
        ...
        if (Array.isArray(data.publicaciones)) {
          for (const publication of data.publicaciones) {
            if (publication.id && publication.urlMaterial) {
              await upsertMaterialLink(publication.id, publication.urlMaterial);
            }
          }
        ...
        setDriveFolderUrl(data.url || null);
        toast({
          title: "Carpetas creadas",
          description: `Se crearon ${selectedPubs.length} carpeta(s) y se vincularon los enlaces de material.`,
        });
      } catch (err: any) {
    ...
    try {
      const response = await fetch(GOOGLE_SCRIPT_URL, {
        ...
        body: JSON.stringify({
          ...
          urlMaterial: buildDriveFolderUrl(clientMaterialUrl || ""),
          nombrePaquete: packageMonth || packageName || "",
          publicaciones: publications.map((publication) => ({
            id: publication.id,
            nombre: publication.name,
          })),
        }),
      ...
      if (Array.isArray(data.publicaciones)) {
        for (const publication of data.publicaciones) {
          if (publication.id && publication.urlMaterial) {
            const wasLinked = await upsertMaterialLink(
              publication.id,
              publication.urlMaterial,
            );
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <PublicationForm
                ...
                publicationDates={publications.map((publication) => new Date(publication.date))}
                clientId={clientId}
                existingPublications={publications.map((publication) => ({
              ...
              <div className="pt-2 border-t space-y-2">
                {driveStatus === "idle" && (
                  <Button
                    onClick={() => setShowDriveFolderSelection(true)}
                    variant="outline"
                    className="w-full"
                    disabled={isSubmitting || publications.length === 0}
                  >
