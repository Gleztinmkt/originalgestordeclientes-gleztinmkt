...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!(await canManageMetaPublishing(admin, userData.user.id))) return new Response(JSON.stringify({ error: "Solo administradores pueden reintentar publicaciones Meta" }), { status: 403, headers: corsHeaders });
